...
export function InspectionSummary({ data = defaultData }: InspectionSummaryProps) {
  ...
  const categories: { key: keyof SummaryData; label: string; color: string; count: number }[] = [
    { key: 'deleted',   label: 'Deleted',   color: theme.statusColors.deleted,      count: totalDeleted },
    { key: 'added',     label: 'Added',     color: theme.statusColors.added,        count: totalAdded },
    { key: 'modified',  label: 'Modified',  color: theme.statusColors.modified,     count: totalModified },
    // { key: 'misplaced', label: 'Misplaced', color: theme.statusColors.repositioned, count: totalMisplaced },
  ];

  const elementTypes: { label: string; icon: React.ReactNode; key: keyof { text: number; symbol: number; barcode: number; image: number } }[] = [
    { label: 'Text',    icon: <TextIcon />,    key: 'text' },
    { label: 'Symbol',  icon: <SymbolIcon />,  key: 'symbol' },
    { label: 'Barcode', icon: <BarcodeIcon />, key: 'barcode' },
    { label: 'Image',   icon: <ImageIcon />,   key: 'image' },
  ];
    ...
    <div className="bg-white border border-gray-300 p-5">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="text-sm font-bold text-gray-900">
          Total Differences: <span>{totalDiffs}</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-4 gap-8 mb-4 pb-4 border-b border-gray-200">
        {categories.map((cat) => (
          <div key={cat.key} className="flex items-center gap-2">
            <span className="text-sm font-semibold" style={{ color: cat.color === theme.statusColors.deleted ? '#dc2626' : cat.color }}>
              {cat.label}:
            </span>
            <span className="text-sm font-bold" style={{ color: cat.color === theme.statusColors.deleted ? '#dc2626' : cat.color }}>
              {cat.count}
            </span>
          </div>
        ))}
      ...
      {/* Bottom grid: element-type breakdown per category */}
      <div className="grid grid-cols-4 gap-8">
        {categories.map((cat) => (
          <div key={cat.key} className="space-y-1.5">
            {elementTypes.map((el) => (
              <div key={el.key} className="flex items-center text-xs"
                style={{ color: data[cat.key][el.key] > 0 ? (cat.color === theme.statusColors.deleted ? '#dc2626' : cat.color) : undefined }}>
