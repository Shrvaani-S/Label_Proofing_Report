...
function RevisionContent({ report, revision }: { report: MultiRevisionReport; revision: LabelRevision }) {
  return (
...
function AccordionCard({ report, revision, defaultOpen }: {
  report: MultiRevisionReport; revision: LabelRevision; defaultOpen: boolean;
}) {
  const [open, setOpen] = useState(defaultOpen);
  return (
    <div className="border border-gray-300 bg-white">
      <RevisionCardHeader sku={report.sku} revision={revision} interactive open={open} onToggle={() => setOpen(o => !o)} />
      {open && <div className="p-8"><RevisionContent report={report} revision={revision} /></div>}
    </div>
  );
}

function AccordionView({ report }: { report: MultiRevisionReport }) {
  return (
    <div className="space-y-4">
      {report.revisions.map((rev, i) => (
        <AccordionCard key={rev.revisionName} report={report} revision={rev} defaultOpen={i === 0} />
      ))}
    </div>
  );
}
...
function MatrixView({ report }: { report: MultiRevisionReport }) {
          ...
          <div key={rev.revisionName} className="border border-gray-300 bg-white">
            ...
            {openCards.has(i) && (
              <div className="p-8 space-y-6">
                <LabelComparison
                  currentLabelUrl={report.currentLabelUrl} currentLabelName={report.currentLabelName}
                  newLabelUrl={rev.labelUrl} newLabelName={rev.labelName}
                  currentBoxes={report.currentBoxes} newBoxes={rev.boxes}
                />
...
function PrintContent({ report, selectedRevNames, viewMode }: { report: MultiRevisionReport; selectedRevNames: string[]; viewMode: ViewMode }) {
  const selectedRevisions = report.revisions.filter(r => selectedRevNames.includes(r.revisionName));
  ...
  const revNames  = selectedRevisions.map(r => r.revisionName).join(', ');
  const newLabels = selectedRevisions.map(r => r.labelName);

  return (
    <div>
      {/* ── Overview page: only when multiple revisions selected ── */}
        ...
        <div style={{ pageBreakAfter: 'always' }}>
          ...
          <div className="bg-white border border-gray-300 grid" style={{ gridTemplateColumns: '0.7fr 0.7fr 1fr 1.5fr 1.5fr' }}>
            {[
              ...
              { label: 'Label Revision',        value: `${report.currentRevision} → ${revNames}` },
              { label: 'Current Version Label', value: report.currentLabelName },
              { label: 'New Version Labels',    value: newLabels.join(', ') },
            ].map((cell, i, arr) => (
              <div key={i} className={`px-3 py-1.5 ${i < arr.length - 1 ? 'border-r border-gray-300' : ''}`}>
                <div className="text-[10px] uppercase tracking-wide text-gray-500 font-bold mb-1">{cell.label}</div>
                <div className="text-xs text-gray-900 break-all">{cell.value}</div>
      ...
      {/* ── Accordion view: one full page per revision ── */}
      {viewMode === 'accordion' && selectedRevisions.map((rev, i) => (
        <div key={rev.revisionName} style={i > 0 ? { pageBreakBefore: 'always' } : {}}>
          <PrintPageHeader report={report} revision={rev} />
          <div className="p-8">
            {/* Non-interactive header strip (same style as accordion card) */}
            <RevisionCardHeader sku={report.sku} revision={rev} interactive={false} />
            <div className="mt-4">
              <RevisionContent report={report} revision={rev} />
            </div>
          </div>
        </div>
      ))}
      ...
      {/* ── Matrix view ── */}
      {viewMode === 'matrix' && selectedRevisions.length > 0 && (
        <>
          {/* Page 1: single header + requirements table */}
          <div style={{ pageBreakAfter: 'always' }}>
            {selectedRevisions.length === 1
              ? <PrintPageHeader report={report} revision={selectedRevisions[0]} />
              : <PrintMatrixPageHeader report={{ ...report, revisions: selectedRevisions }} />
            }
            <div className="p-8">
              <div className="bg-white border border-gray-300">
                <div className="overflow-x-auto">
                  <table className="w-full border-collapse text-xs">
                    ...
                    <tbody>
                      {allDescs.map((desc, i) => {
                        const base = firstMatch(desc);
                        const anyFail = selectedRevisions.some(rev => lookup[desc][rev.revisionName]?.status === 'Unmatch');
                        return (
                          <tr key={i} className={`border-b border-gray-100 last:border-0 ${anyFail ? 'bg-red-50/40' : ''}`}>
                            <td className="px-3 py-2 text-gray-400 border-r border-gray-100">{i + 1}</td>
                            <td className="px-3 py-2 text-gray-700 border-r border-gray-100">{base?.elementType ?? '—'}</td>
          ...
          {/* One page per revision: header → label image → changes */}
          {selectedRevisions.map(rev => (
            <div key={rev.revisionName} style={{ pageBreakAfter: 'always' }}>
              <PrintPageHeader report={report} revision={rev} />
              <div className="p-8 space-y-6">
                <LabelComparison
                  currentLabelUrl={report.currentLabelUrl} currentLabelName={report.currentLabelName}
...
function PdfModal({
  ...
  const [selected, setSelected] = useState<Set<string>>(new Set(report.revisions.map(r => r.revisionName)));

  const toggle = (name: string) =>
    setSelected(prev => { const s = new Set(prev); s.has(name) ? s.delete(name) : s.add(name); return s; });

  const toggleAll = () =>
    setSelected(selected.size === report.revisions.length ? new Set() : new Set(report.revisions.map(r => r.revisionName)));
      ...
      <div className="bg-white border border-gray-200 shadow-lg w-96">
        ...
        <div className="px-6 py-4 space-y-3">
          ...
          {report.revisions.map(rev => {
            const { matched, total } = matchCount(rev);
            return (
              <label key={rev.revisionName} className="flex items-center gap-3 text-xs cursor-pointer">
                <input
                  type="checkbox"
                  checked={selected.has(rev.revisionName)}
                  onChange={() => toggle(rev.revisionName)}
                  className="w-3.5 h-3.5 accent-[#D71500]"
                />
...
function ViewNav({ active, onChange }: { active: ViewMode; onChange: (v: ViewMode) => void }) {
...
export default function MultiRevisionSample() {
  ...
  const [selectedRevNames,  setSelectedRevNames]  = useState<string[]>(REPORT.revisions.map(r => r.revisionName));
      ...
      <div className="min-h-screen bg-[#f3f4f6]">
        ...
        <div className="max-w-[1600px] mx-auto">
          ...
          <div className="print:hidden">
            <ViewNav active={viewMode} onChange={setViewMode} />
            <div className="p-8">
              {viewMode === 'accordion' && <AccordionView report={REPORT} />}
              {viewMode === 'matrix'    && <MatrixView    report={REPORT} />}
            </div>
          </div>

          {/* Print only: active view only, selected revisions only */}
          <div className="hidden print:block">
            <PrintContent report={REPORT} selectedRevNames={selectedRevNamesRef.current} viewMode={viewModeRef.current} />
          </div>
        </div>
