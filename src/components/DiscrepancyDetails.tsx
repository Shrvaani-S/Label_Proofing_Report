...
import type { DiscrepancyCategory, ChangeType } from '@/common/types';

// Internal type used for rendering (adds color)
interface CategoryDisplay {
  title: string;
  color: string;
  items: { changeType: ChangeType; value: string }[];
}
...
export function DiscrepancyDetails({ categories: propCategories }: DiscrepancyDetailsProps) {
  ...
  const categories: CategoryDisplay[] = source.map(c => ({
    title: c.title,
    color: colorForCategory(c.items),
    items: c.items,
  }));
        ...
        return (
          <div key={category.title} className="bg-white border border-gray-300">
            ...
            >
              ...
              <svg
                className={`w-5 h-5 text-gray-500 transition-transform print:hidden ${isOpen ? 'rotate-180' : ''}`}
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
                    ...
                    group.values.map((val, vi) => (
                      <tr key={`${group.changeType}-${vi}`} className="border-b border-gray-200 last:border-0">
                        <td className="px-4 py-2 border-r border-gray-200 align-top">
                          <Badge type={group.changeType as 'Modified' | 'Added' | 'Deleted' | 'Repositioned' | 'Misplaced'} />
                        </td>
